<template>
  <li class="task-item">
    ...
    <label
      ...
      :class="{'task-item__label--strike': task.completed !== null, 'task-starred': task.starred === '1'}"
      :for="'task' + task.id"
      :style="{fontWeight: task.starred === '1' ? 700 : 400}"
    >
      {{ task.description }}
      <span
        class="task-item__strike"
        :class="{'task-item__strike--show': task.completed !== null}"
      ></span>
    </label>
